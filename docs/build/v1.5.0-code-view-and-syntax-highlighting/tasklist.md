...
| ID  | Task             | Description                             | Dependencies | Status | Assigned To |
|-----|------------------|-----------------------------------------|--------------|--------|-------------|
| 7.1 | Bump version in package.json | Updated `version` from `1.4.0` to `1.5.0` and `lastUpdated` to `2026-04-17` in `package.json`. `/api/version` serves both to the header, welcome modal, settings, and About modal so no per-file version strings needed updating. | None | 🟢 Completed | AGENT |
| 7.2 | Bump version in index.html | No change needed — `index.html` has no hardcoded version string. The header, welcome, settings, and About modal all read from `/api/version` (wired in v1.4.0). Verified via grep: no `v1.4.0` or `1.4.0` references anywhere in index.html. | None | 🟢 Completed | AGENT |
| 7.3 | Update README.md | Bumped version badge from 1.4.0 to 1.5.0. Added a new "Code View — grab the API call for your current config" section explaining the feature + Original prompt toggle. Updated the "Digging deeper" paragraph to mention Prism highlighting + copy button + global Wrap code toggle added to the Full I/O viewer. Updated the release-notes reference line to highlight v1.5.0 instead of v1.3.0. | 7.1 | 🟢 Completed | AGENT |
| 7.4 | Review and update Welcome slideshow | Reviewed slideshow content (slides 1–4 in `public/index.html`). Honest assessment: Code View is a power-user utility feature and doesn't warrant a slide addition — it would add noise to the onboarding narrative which is focused on the advisor strategy itself. Users will discover the `</>` icon naturally in the top-nav. Version string in the welcome header auto-updates from package.json. No changes. | 7.1 | 🟢 Completed | AGENT |
| 7.5 | Review and update About modal | Verified via code path: About modal's version and `last-updated` text are populated by the `/api/version` fetch on page load (see `app.js` around the aboutVersionEl / aboutLastUpdatedEl handlers). Bumping `package.json` in 7.1 flows through automatically. No manual About-modal content changes needed; the tagline is advisor-tool-general, not version-specific. | 7.1 | 🟢 Completed | AGENT |
| 7.6 | Update release-notes.md | Added v1.5.0 entry at top with ToC link. Covers Key Features (Code View, Prism, Copy buttons, Wrap toggle, Original prompt, top-nav redesign), Enhancements (sentinel stripping, tab keyboard nav, consistent modal height, scroll hoisting, ADVISOR_BETA constant), and Other Notes (Prism self-host rationale + version, E2E validation, TS/Python validation caveat). | 7.1 | 🟢 Completed | AGENT |
| 7.7 | Update feature backlog status | Changed v1.5.0 section header from `🔴 Not Started` to `🟢 Completed`. Marked F33, F34, F35 as `🟢 Completed` with descriptions updated to reflect what shipped (including the polish items added mid-version). | 7.6 | 🟢 Completed | AGENT |
| 7.8 | Update cody.json | Set `version` to `1.5.0` and `updatedAt` to `2026-04-17` in the `cody-product-builder` section. | 7.1 | 🟢 Completed | AGENT |
| 7.9 | Write retrospective | Created `docs/build/v1.5.0-code-view-and-syntax-highlighting/retrospective.md`. Captures what went well (mockup-first decisions, Phase 3 foundation, honest pushback on API-key inlining, E2E curl validation, nested-scroll diagnosis, lazy-init TDZ fix), what didn't (Prism `<code>` white-space oversight, two-iteration heredoc escaping, three different wrap toggles before settling, TS/Python not executed, `node --check` insufficient for TDZ), lessons learned, and action items for future versions. | 7.7 | 🟢 Completed | AGENT |
| 7.10 | Final test | USER confirmed final smoke test passed: header shows v1.5.0, About modal shows `Last updated: 2026-04-17`, Code View features work end-to-end. | 7.1-7.9 | 🟢 Completed | USER |
| 7.11 | Commit phase 7 | USER committing the version bump and finalization, then pushing v1.5.0 to GitHub (Phase 4–7 local commits + final phase 7 all go up together). | 7.10 | 🟢 Completed | USER |
