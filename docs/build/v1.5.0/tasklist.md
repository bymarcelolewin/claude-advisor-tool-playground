...
| ID  | Task             | Description                             | Dependencies | Status | Assigned To |
|-----|------------------|-----------------------------------------|--------------|--------|-------------|
| 4.1 | Add `</>` button to top-nav | Add a new `<button>` in the top-nav cluster of `public/index.html`, placed left of the info icon. Glyph `</>` rendered in `var(--mono)`. `aria-label="Code View"`, `title="View code for current configuration"`. Same DOM structure / sizing as the existing ⓘ and ⚙ buttons. | None | 🔴 Not Started | AGENT |
| 4.2 | Add Code View modal markup | Add modal markup in `public/index.html` mirroring the `.settings-modal` + `.settings-card` pattern. Header with `</>` glyph + title + subtitle + close button. Empty `<div id="code-view-pill-rail">`, tab row, code body with three `<div class="tab-panel">` slots, footer. | None | 🔴 Not Started | AGENT |
| 4.3 | Style Code View modal | Add CSS in `public/styles.css` for the Code View modal. Reuse existing modal classes where possible. Add: `.code-view-pill-rail`, `.pill`, `.pill.exec`, `.pill.adv`, `.code-view-tabs`, `.code-view-tab`, `.code-view-tab.active`, `.tab-panel`, `.tab-panel.active`, `.code-view-footer`. Match Mockup A spacing, colors, and typography. | 4.2 | 🔴 Not Started | AGENT |
| 4.4 | Wire open/close mechanics | In `public/app.js`: clicking `</>` button opens the modal. Pressing `Esc`, clicking the backdrop, or clicking the close button closes it. Mirror the existing Settings modal handlers. Return focus to the `</>` button on close. | 4.1, 4.2 | 🔴 Not Started | AGENT |
| 4.5 | Implement tab switching | Click handler on tab buttons toggles the `.active` class on the clicked tab and the matching panel. ARIA: tablist role on container, tab role + `aria-selected` on tabs. Default active tab: TypeScript. | 4.2 | 🔴 Not Started | AGENT |
| 4.6 | Add focus trap | While modal is open, trap focus inside the modal (Tab cycles within). Reuse existing focus-trap logic from Settings modal if present, otherwise implement minimal version. | 4.4 | 🔴 Not Started | AGENT |
| 4.7 | Test phase 4 | USER tests: button appears in correct position with correct styling; modal opens/closes via button, Esc, backdrop, and close icon; tabs switch; focus returns to `</>` button on close. Snippets are still placeholders — that's expected. | 4.1-4.6 | 🔴 Not Started | USER |
| 4.8 | Commit phase 4 | USER commits phase 4 to git. | 4.7 | 🔴 Not Started | USER |
